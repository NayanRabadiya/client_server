...
def save_board_to_file(board, path: str):
    data = {
        "board": [[cell.to_dict() if cell else None for cell in row] for row in board]
    }
    ...
    for y, row in enumerate(board):
        for x, p in enumerate(row):
            if (
                ...
                and p.owner == player
                and p.side == "stone"
                and is_own_score_cell(x, y, player, rows, cols, score_cols)
            ):
                n += 1
    return n
...
def compute_final_scores(
    ...
    if winner in ("circle", "square"):
        ...
        for player in ("circle", "square"):
            ...
            margin = 39.0 + (
                (float(n_self) + float(m_self) / 10.0)
                - (float(n_opp) + float(m_opp) / 10.0)
            )
...
def validate_and_apply_move(
    ...
    if action == "move":
        ...
        if piece.side == "stone":
            ...
            flow = get_river_flow_destinations(
                board, fx, fy, fx, fy, player, rows, cols, score_cols
            )
        ...
        piece.orientation = (
            "horizontal" if piece.orientation == "vertical" else "vertical"
        )
        flow = get_river_flow_destinations(
            board, fx, fy, fx, fy, player, rows, cols, score_cols
        )
            ...
            if p.side == "stone":
                ...
                for ori in ("horizontal", "vertical"):
                    ...
                    if not any(
                        ...
                        moves.append(
                            {"action": "flip", "from": [x, y], "orientation": ori}
                        )
            else:
...
# ---------------- Win check ----------------
def check_win(
    board: List[List[Optional[Piece]]], rows: int, cols: int, score_cols: List[int]
) -> Optional[str]:
    top = top_score_row()
    ...
    for x in score_cols:
        if in_bounds(x, top, rows, cols):
            p = board[top][x]
            if p and p.owner == "circle" and p.side == "stone":
                ccount += 1
        if in_bounds(x, bot, rows, cols):
            q = board[bot][x]
            if q and q.owner == "square" and q.side == "stone":
                scount += 1
    if ccount >= WIN_COUNT:
        return "circle"
    if scount >= WIN_COUNT:
...
def draw_board(
    screen, board, rows, cols, score_cols, selected, highlights, msg, timers, current
):
    screen.fill(BG)

    # Draw background with gradient effect
    board_rect = pygame.Rect(
        MARGIN - 30, MARGIN - 30, cols * CELL + 60, rows * CELL + 60
    )
    ...
    for x in score_cols:
        r = pygame.Rect(
            MARGIN + x * CELL - CELL // 2, MARGIN + top * CELL - CELL // 2, CELL, CELL
        )
        # Subtle background
    ...
    for x in score_cols:
        r = pygame.Rect(
            MARGIN + x * CELL - CELL // 2, MARGIN + bot * CELL - CELL // 2, CELL, CELL
        )
        # Subtle background
        s = pygame.Surface((r.w, r.h), pygame.SRCALPHA)
    ...
    for y in range(rows):
        for x in range(cols):
            ...
            pygame.draw.circle(
                shadow_surf, SHADOW_COLOR, (CELL // 2 + 1, CELL // 2 + 1), CELL // 2 - 4
            )
            ...
            if p.side == "stone":
                ...
                river_color = (
                    RIVER_FILL_CIRCLE if p.owner == "circle" else RIVER_FILL_SQUARE
                )
                ...
                if p.orientation == "horizontal":
                    ...
                    for i in range(2):
                        arrow_x = (
                            cx - river_length // 2 + 8 + i * (river_length - 16) // 1
                        )
                    ...
                    for i in range(2):
                        arrow_y = (
                            cy - river_length // 2 + 8 + i * (river_length - 16) // 1
                        )
    ...
    msg_bg_rect = pygame.Rect(
        10, rows * CELL + MARGIN + 10, screen.get_width() - 20, 35
    )
...
def run_gui(
    ...
    pygame.display.set_caption(
        f"🎮 River and Stones - {mode.upper()} Mode"
    )  # ADDED: set window caption
    ...
    while True:
        ...
        if players[current] == "ai" and not winner and not game_over:
            ...
            if timers[current] <= 0:
                ...
                if move:
                    ok, info = validate_and_apply_move(
                        board, move, current, rows, cols, score_cols
                    )
                    ...
                    if ok:
                        ...
                        turn_start = (
                            time.time()
                        )  # NEW: reset timer when switching to next (human) turn
                    else:
                        current = opponent(current)
                        turn_start = time.time()
                else:
        ...
        for ev in pygame.event.get():
            ...
            if ev.type == pygame.KEYDOWN:
                ...
                if selected and ev.key == pygame.K_m:
                    action_mode = "move"
                    info = compute_valid_targets(
                        board, selected[0], selected[1], current, rows, cols, score_cols
                    )
                ...
                if selected and ev.key == pygame.K_r:
                    ...
                    if p and p.owner == current and p.side == "river":
                        ...
                        ok, info = validate_and_apply_move(
                            board, m, current, rows, cols, score_cols
                        )
                        msg = info
                        if ok:
                            w = check_win(board, rows, cols, score_cols)
            ...
            if ev.type == pygame.MOUSEBUTTONDOWN and ev.button == 1:
                ...
                if not in_bounds(rx, ry, rows, cols):
                    continue
                ...
                if selected is None:
                    ...
                    if action_mode == "move":
                        info = compute_valid_targets(
                            board, sx, sy, current, rows, cols, score_cols
                        )
                        ...
                        if push_stage == 0 or push_stage is None:
                            if (rx, ry) not in own_finals:
                                ...
                                pushed_options = [
                                    pf for of, pf in push_pairs if of == push_candidate
                                ]
        ...
        if game_over and game_scores is None:
            game_scores = compute_final_scores(
                ...
                score_cols,
                remaining_times={
                    "circle": timers["circle"],
                    "square": timers["square"],
                },
            )
            if winner in ("circle", "square"):
...
def run_cli(
    ...
    while True:
        ...
        print(
            f"Remaining time — Circle: {format_time(timers['circle'])} | Square: {format_time(timers['square'])}"
        )
        print(f"{'='*30}")
        ...
        if players[current] == "ai":
            ...
            if timers[current] <= 0:
                winner = opponent(current)
                print(
                    f"{current.title()} timed out while thinking. {winner.title()} wins!"
                )
                break
            ...
            if move is None:
                ...
                continue
            ok, msg = validate_and_apply_move(
                board, move, current, rows, cols, score_cols
            )
            ...
            print(
                "  Flip: {'action':'flip','from':[x,y],'orientation':'horizontal/vertical'}"
            )
    ...
    print(
        f"Final Scores -> Circle: {final_scores['circle']:.1f} | Square: {final_scores['square']:.1f}\n"
    )
...
def main():
    ...
    ap.add_argument(
        "--circle", choices=["random", "student", "student_cpp"], default="random"
    )
    ap.add_argument(
        "--square", choices=["random", "student", "student_cpp"], default="random"
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=1.0,
        help="Time per player in minutes (default: 1.0)",
    )
    args = ap.parse_args()
    ...
    if args.nogui:
        run_cli(
            args.mode, args.circle, args.square, args.load, rows, cols, time_per_player
        )
    else:
        run_gui(
            args.mode, args.circle, args.square, args.load, rows, cols, time_per_player
