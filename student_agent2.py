...
def generate_heuristic_moves(board, player, rows, cols, score_cols):
    ...
    moves_with_to = moves_with_to[:10]
...
def evaluate_simple(board, player, rows, cols, score_cols):
    ...
    """
    weights = load_weights("weights_learning.json")
    opponent = get_opponent(player)
...
def minValue(board, current_player, original_player, rows, cols, score_cols, alpha, beta, ply):
    ...
    for move in moves:
        ...
        if success:
            childVal, childFeat = maxValue(child_board, opponent, original_player, rows, cols, score_cols, alpha, beta, ply)
            if childVal < minVal:
                minVal = childVal
...
def maxValue(board, current_player, original_player, rows, cols, score_cols, alpha, beta, ply):
    ...
    for move in moves:
        success, child_board = simulate_move(board, move, current_player, rows, cols, score_cols)
        if success:
            childVal, childFeat = minValue(child_board, opponent, original_player, rows, cols, score_cols, alpha, beta, ply-1)
            if childVal > maxVal:
                maxVal = childVal
