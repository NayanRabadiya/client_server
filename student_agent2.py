...
def validate_and_apply_move(
    ...
    # -------------------
        ...
        if not all(
            in_bounds(x, y, rows, cols) for x, y in [(fx, fy), (tx, ty), (px, py)]
        ):
            return False, "out of bounds"
        ...
        valid_info = compute_valid_targets(
            board, fx, fy, player, rows, cols, score_cols
        )
...
def load_weights(file_path="weights_learning.json"):
...
def get_features(board, player, rows, cols, score_cols):
    ...
    features = {
        ...
        "move_count": len(moves),
        "opp_move_count": len(opp_moves),
        "goal_side_dist_row_moves": eval_moves_dist_to_goal_side_row(
            board, player, rows, cols, score_cols, moves
        ),
        "opp_goal_side_dist_row_moves": eval_moves_dist_to_goal_side_row(
            board, opponent, rows, cols, score_cols, opp_moves
        ),
        "goal_side_dist_col_moves": eval_moves_dist_to_goal_side_col(
            board, player, rows, cols, score_cols, moves
        ),
        "opp_goal_side_dist_col_moves": eval_moves_dist_to_goal_side_col(
            board, opponent, rows, cols, score_cols, opp_moves
        ),
    }
...
def minimax_move(board, player, rows, cols, score_cols):
    maxVal = -float("inf")
    maxMove = None
    bestFeatures = None
    opponent = get_opponent(player)
    ...
    update_weights(curr_features, bestFeatures)
    return maxMove


def minValue(
    board, current_player, original_player, rows, cols, score_cols, alpha, beta, ply
):
